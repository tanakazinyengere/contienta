...
const TABS: { id: AppTabId; label: string; Icon: typeof Search }[] = [
  ...
  { id: "engine", label: "Engine", Icon: Sparkles },
...
const BottomNav = ({ active, onChange }: BottomNavProps) => {
  return (
    <nav
      className="fixed bottom-0 inset-x-0 z-40 border-t border-border bg-background/80 backdrop-blur-xl"
        ...
        {TABS.map(({ id, label, Icon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => onChange(id)}
              className="relative flex flex-col items-center justify-center gap-1 py-2.5 press-effect min-h-[56px]"
              aria-label={label}
            >
              {isActive && (
                <motion.span
